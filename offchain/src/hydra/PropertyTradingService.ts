...
import { HydraClient, HydraUTxO, TransactionResult } from './HydraClient';
...
export class PropertyTradingService extends EventEmitter {
  ...
  private readonly TPS_WINDOW_SIZE = 60; // 60 seconds

  constructor(hydra: HydraClient) {
    super();
    this.hydra = hydra;
    this.stats = {
      totalTrades: 0,
   ...
   */
  getAvailableFractions(): PropertyFraction[] {
    const fractions: PropertyFraction[] = [];
   ...
   */
  getFractionsAtAddress(address: string): PropertyFraction[] {
  ...
  async placeSellOrder(
    ...
    const hasEnough = sellerFractions.some(
      (f) => f.policyId === policyId && f.assetName === assetName && f.quantity >= quantity
    );
    ...
    if (orderBook) {
      if (order.buyer) {
        orderBook.bids = orderBook.bids.filter(
          (b) => b.address !== order.buyer || b.quantity !== order.quantity
        );
      }
      if (order.seller) {
        orderBook.asks = orderBook.asks.filter(
          (a) => a.address !== order.seller || a.quantity !== order.quantity
        );
      }
    }
  ...
  private async matchOrders(propertyId: string): Promise<void> {
  ...
  getOrderBook(propertyId: string): OrderBook | undefined {
  ...
  private updateStats(volume: bigint, latencyMs: number): void {
  ...
  private handleTxConfirmed(message: any): void {
  ...
  private handleTxFailed(message: any): void {
    for (const [orderId, order] of this.activeOrders) {
      if (order.status === 'submitted') {
        order.status = 'failed';
  ...
  private isPropertyFractionToken(policyId: string, assetName: string): boolean {
    // Check if asset name starts with CIP-68 user token prefix (000de140)
    return assetName.startsWith('000de140');
  }

  private extractPropertyId(assetName: string): string {
  ...
  private getPropertyMetadata(utxo: HydraUTxO): PropertyMetadata | undefined {
